...
public class PortfolioService {
    ...
    public List<PortfolioResponseDto> getPortfolios(Long profileId, boolean isLoggedIn) {
        List<PortfolioResponseDto> portfolios = portfolioRepository
                .findByFreelancerProfileIdOrderBySortOrder(profileId)
                .stream()
    ...
    public PortfolioResponseDto createPortfolio(Member member, Long profileId,
                                                MultipartFile image,
                                                PortfolioRequestDto dto) throws IOException {
    ...
    // 포트폴리오 추가 이미지 업로드
    @Transactional
    public PortfolioResponseDto addImage(Member member, Long portfolioId,
        ...
        int nextOrder = portfolio.getImages().size();
    ...
    // 포트폴리오 추가 이미지 삭제
    @Transactional
    public void deleteImage(Member member, Long portfolioId, Long imageId) {
        Portfolio portfolio = portfolioRepository.findById(portfolioId)
                .orElseThrow(() -> new IllegalArgumentException("포트폴리오를 찾을 수 없습니다."));
        if (!portfolio.getFreelancerProfile().getMember().getId().equals(member.getId())) {
            throw new IllegalStateException("권한이 없습니다.");
        ...
        r2FileUploadService.delete(image.getImageUrl());
        portfolioImageRepository.delete(image);
    }

    // 포트폴리오 수정
    @Transactional
    public PortfolioResponseDto updatePortfolio(Member member, Long portfolioId,
                                                MultipartFile image,
                                                PortfolioRequestDto dto) throws IOException {
        Portfolio portfolio = portfolioRepository.findById(portfolioId)
                .orElseThrow(() -> new IllegalArgumentException("포트폴리오를 찾을 수 없습니다."));
        ...
        if (image != null && !image.isEmpty()) {
            FileValidator.validate(image);
            r2FileUploadService.delete(imageUrl);
            imageUrl = r2FileUploadService.upload(image, "portfolios");
        }

        portfolio.update(imageUrl, dto.getDescription(), dto.getSortOrder(),
                dto.getStartDate(), dto.getEndDate(),
                dto.getClient(), dto.getIndustry(), dto.getPurpose());
