    ...
    <div className="flex flex-col min-h-screen bg-[#fbf9f2]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        <div className="flex flex-col lg:flex-row gap-8">
          <MySidebar name={name} email={email} profileImg={profileImg} role={role} onLogout={handleLogout} />

          <main className="flex-1 space-y-6">
            <div className="flex items-center justify-between">
              <h1 className="font-[var(--font-display)] text-2xl font-semibold text-[#1b1c18]">내 구인글</h1>
              <Link href="/jobs/write">
                <button className="text-sm font-medium text-[#4f6231] hover:underline">새 구인글 작성</button>
              </Link>
            </div>
            ...
            {loading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-white rounded-2xl border border-[#efeee7] p-6">
              ...
              <div className="bg-white rounded-2xl border border-[#efeee7] p-12 text-center">
                <p className="text-[#75786c] text-sm mb-4">등록한 구인글이 없습니다.</p>
                <Link href="/jobs/write" className="text-sm text-[#4f6231] hover:underline font-medium">
                  구인글 작성하기
                </Link>
              </div>
            ) : (
                ...
                {posts.map((post) => (
                  <Link key={post.id} href={`/jobs/${post.id}`}>
                    <div className="bg-white rounded-2xl border border-[#efeee7] p-6 hover:border-[#c5c8ba] transition-colors cursor-pointer">
                      ...
                      <h2 className="text-sm font-semibold text-[#1b1c18] mb-2">{post.title}</h2>
                      <div className="flex gap-4 text-xs text-[#75786c]">
                        <span>예산: {post.budget != null ? `${post.budget.toLocaleString()}원` : "협의"}</span>
                        <span>웨딩일: {new Date(post.weddingDate).toLocaleDateString("ko-KR")}</span>
