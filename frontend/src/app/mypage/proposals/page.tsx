...
export default function MyProposalsPage() {
  ...
  useEffect(() => {
    ...
    const fetchProposals = async () => {
      try {
        const res = await authFetch(`${API_BASE_URL}/api/v1/members/me/proposals`);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <main className="flex-1 space-y-6">
            <h1 className="font-[var(--font-display)] text-2xl font-semibold text-[#1b1c18]">내 제안서</h1>
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border border-[#efeee7] p-12 text-center">
                <p className="text-[#75786c] text-sm mb-4">제출한 제안서가 없습니다.</p>
                <Link href="/jobs" className="text-sm text-[#4f6231] hover:underline font-medium">
                  구인글 둘러보기
                </Link>
              </div>
            ) : (
              <div className="space-y-4">
                {proposals.map((proposal) => (
                  <div key={proposal.id} className="bg-white rounded-2xl border border-[#efeee7] p-6">
                    ...
                    <p className="text-sm text-[#45483d] whitespace-pre-wrap mb-3 line-clamp-3">{proposal.content}</p>
