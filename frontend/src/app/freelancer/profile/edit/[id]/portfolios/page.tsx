...
export default function FreelancerPortfolioEditPage() {
  ...
  useEffect(() => {
    if (!getAccessToken()) {
      router.push(`/login?redirect=/freelancer/profile/edit/${id}/portfolios`);
      return;
    }
  ...
  const handleExistingImageDelete = async (imageId: number) => {
    ...
    try {
      await authFetch(
        `${API_BASE_URL}/api/freelancers/${id}/portfolios/${editingPortfolio.id}/images/${imageId}`,
        { method: "DELETE" },
      );
      setEditingPortfolio((prev) =>
        prev
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      await authFetch(
        `${API_BASE_URL}/api/freelancers/${id}/portfolios/${editingPortfolio.id}`,
      ...
      for (const img of editingPortfolio.newImages) {
        ...
        const res = await authFetch(
          `${API_BASE_URL}/api/freelancers/${id}/portfolios/${editingPortfolio.id}/images`,
          { method: "POST", body: imgFormData },
        ).catch(() => {
          console.warn("추가 이미지 업로드 실패");
          return null;
        });
        if (res?.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/profile/${id}/portfolios`);
    } catch (error) {
      setErrorMessage(
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#fbf9f2]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        ...
        {errorMessage && (
          <p className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-500 mb-5">
            {errorMessage}
          </p>
        )}
            ...
            <div className="flex flex-col gap-3">
              {currentExisting.map((item) => (
                <div
                  ...
                  className="flex items-center gap-3 p-3 border border-[#efeee7] rounded-xl"
                >
        ...
        <div className="flex justify-end gap-3 mt-5">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push(`/profile/${id}/portfolios`)}
