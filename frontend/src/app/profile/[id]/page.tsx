    ...
    const fetchAll = async () => {
      try {
        ...
        const [profileRes, portfolioRes, reviewRes] = await Promise.all([
          ...
          fetch(`/api/freelancers/${params.id}/portfolios`),
          fetch(`/api/freelancers/${params.id}/reviews`),
        ]);
        ...
        <div className="flex flex-col min-h-screen bg-[#fbf9f2]">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-[#75786c]">프로필을 불러오는 중입니다...</p>
            </div>
          </div>
          <Footer />
        </div>
  ...
  if (error || !profile) {
    return (
        <div className="flex flex-col min-h-screen bg-[#fbf9f2]">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <p className="text-lg font-semibold text-[#1b1c18] mb-2">프로필을 찾을 수 없습니다</p>
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm text-[#45483d] ml-1">{reviews.length} 리뷰</span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link href={`/reserve/${params.id}`} className={cn(buttonVariants(), "bg-[#4f6231] text-white hover:bg-[#677b47] rounded-xl px-6")}>
                    예약하기
                  </Link>
                </div>
              </div>
              ...
              {portfolios.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
                    {portfolios.map((portfolio) => (
                        <div key={portfolio.id} className="relative aspect-[4/3] rounded-xl overflow-hidden group">
                          <Image
            ...
            <TabsContent value="reviews" className="mt-0">
              {reviews.length === 0 ? (
                  ...
                  <div className="space-y-6 max-w-2xl">
                    {reviews.map((review) => (
                        <div key={review.id} className="bg-white rounded-2xl p-6 border border-[#efeee7]">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                              <p className="font-semibold text-[#1b1c18] text-sm">{review.memberName}</p>
                              <p className="text-xs text-[#75786c]">{review.createdAt.slice(0, 10)}</p>
                            </div>
                          ...
                          <p className="text-sm text-[#45483d] leading-relaxed">{review.content}</p>
                        </div>
                    ))}
                  </div>
              )}
            ...
            <TabsContent value="about" className="mt-0">
              <div className="max-w-2xl">
                ...
                <div className="grid grid-cols-2 gap-4 mt-8 p-5 bg-[#f5f4ec] rounded-2xl">
                  <div className="text-center">
                    <p className="font-[var(--font-display)] text-2xl font-bold text-[#4f6231]">{profile.careerYears}년</p>
                    <p className="text-xs text-[#75786c]">경력</p>
                  </div>
                  <div className="text-center">
                    <p className="font-[var(--font-display)] text-2xl font-bold text-[#4f6231]">{reviews.length}개</p>
